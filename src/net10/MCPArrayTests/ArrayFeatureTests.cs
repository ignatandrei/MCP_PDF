...
"""
    ...
    public async Task VerifyArrayToonToHTML(string jsonArray, string htmlResult)
    {
...
"""
    ...
    public async Task VerifyArrayToMarkdown(string jsonArray, string MDResult)
    {
        await Runner.RunScenarioAsync(
            _ => Given_I_have_an_json_array_as_string(jsonArray),
            _ => When_I_Convert_ToMarkdown(),
            _ => Then_the_result_should_be(MDResult)
        );
    }
...
"""
    ...
    private async Task When_I_Convert_ToMarkdown()
    {
        var loggerFactory = LoggerFactory.Create(builder => builder.AddConsole());
        var logger = loggerFactory.CreateLogger<Exporter>();
        //_logger  = new NullLogger<ArrayToAny>();
        htmlResult = await (new Exporter(logger)).ConvertJsonArrayToMarkdown(arrToTest);
        return;
    }
    ...
    private async Task Then_the_result_should_be(string markdown)
    {
        ...
        var min = Math.Min(markdown.Length, htmlResult.Length);
        htmlResult.Length.ShouldBe(min, $"Length mismatch: expected {markdown.Length}, got {htmlResult.Length}");
        markdown.Length.ShouldBe(min);
        for (int i = 0; i < min; i++)
        {
            htmlResult[i].ShouldBe(markdown[i], $"Mismatch at index {i}: expected '{markdown[i]}', got '{htmlResult[i]}'");
        }
        htmlResult.ShouldBe(markdown);
        return;
    }
